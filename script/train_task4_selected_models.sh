#!/bin/bash
# Run specified Task 4 models N times each and average final classification metrics.
#
# Usage:
#   bash script/train_task4_selected_models.sh lstm gru bigru_meanmax
#   N_RUNS=10 EPOCHS=200 bash script/train_task4_selected_models.sh bilstm layernorm_bigru
...
if [ "$#" -eq 0 ] || [ "$1" = "-h" ] || [ "$1" = "--help" ]; then
  echo "Usage: bash script/train_task4_selected_models.sh <model1> [model2 ...]"
  echo ""
  echo "Example:"
  echo "  bash script/train_task4_selected_models.sh lstm gru bigru_meanmax"
  echo "  N_RUNS=10 EPOCHS=200 bash script/train_task4_selected_models.sh bilstm layernorm_bigru"
  exit 0
fi
...
mkdir -p results
RESULTS_FILE="results/task4_selected_models_${TIMESTAMP}.txt"
{
  echo "Task 4 - Selected Models Test Metrics Summary"
  echo "Generated: $(date)"
  echo "N_RUNS=$N_RUNS"
  echo "Models: $*"
  echo ""
...
for model in "$@"; do
  ...
  echo ""
  echo "=========================================="
  echo "Training $model for Task 4 ($N_RUNS runs, result averaged)"
  echo "=========================================="
  run_n_times_and_average "$RESULTS_FILE" "$model" python script/train_task4.py \
    --model "$model" \
    --epochs "$EPOCHS" \
