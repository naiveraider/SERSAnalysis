#!/bin/bash
# Run the original Mamba model plus Mamba pooling ablations in parallel for Task 3.
...
if [ "${1:-}" = "-h" ] || [ "${1:-}" = "--help" ]; then
  echo "Usage: bash script/train_task3_mamba_ablation_parallel.sh"
  echo ""
  echo "Runs 4 jobs in parallel for Task 3:"
...
SELECTED_SCRIPT="script/train_task3_selected_models.sh"
SUMMARY_FILE="results/task3_mamba_ablation_parallel_${TIMESTAMP}.txt"
...
start_job \
  "mamba" \
  "results/log/mamba_task3_${TIMESTAMP}.log" \
  env \
...
start_ablation_job \
  ...
  "mamba_mean" \
  "${JOB_DEVICES[1]}" \
  "results/log/mamba_mean_task3_${TIMESTAMP}.log"

start_ablation_job \
  "mamba_max" \
  "mamba_max" \
  "${JOB_DEVICES[2]}" \
  "results/log/mamba_max_task3_${TIMESTAMP}.log"

start_ablation_job \
  "mamba_meanmax" \
  "mamba_meanmax" \
  "${JOB_DEVICES[3]}" \
  "results/log/mamba_meanmax_task3_${TIMESTAMP}.log"
...
{
  echo "Task 3 - Parallel Mamba Ablation Launcher"
  echo "Generated: $(date)"
  echo "Wrapper log: $WRAPPER_LOG"
  echo "Python: $PYTHON_BIN"
  echo "N_RUNS=$N_RUNS"
