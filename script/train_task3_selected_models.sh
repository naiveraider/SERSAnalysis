#!/bin/bash
# Run specified Task 3 models N times each and average final classification metrics.
#
# Usage:
#   bash script/train_task3_selected_models.sh lstm gru bigru_meanmax
#   N_RUNS=10 EPOCHS=100 bash script/train_task3_selected_models.sh bilstm layernorm_bigru
...
if [ "$#" -eq 0 ] || [ "$1" = "-h" ] || [ "$1" = "--help" ]; then
  echo "Usage: bash script/train_task3_selected_models.sh <model1> [model2 ...]"
  echo ""
  echo "Example:"
  echo "  bash script/train_task3_selected_models.sh lstm gru bigru_meanmax"
  echo "  N_RUNS=10 EPOCHS=100 bash script/train_task3_selected_models.sh bilstm layernorm_bigru"
  exit 0
fi
...
mkdir -p results
RESULTS_FILE="results/task3_selected_models_${TIMESTAMP}.txt"
{
  echo "Task 3 - Selected Models Test Metrics Summary"
  echo "Generated: $(date)"
  echo "N_RUNS=$N_RUNS"
  echo "Models: $*"
  echo ""
...
for model in "$@"; do
  ...
  echo ""
  echo "=========================================="
  echo "Training $model for Task 3 ($N_RUNS runs, result averaged)"
  echo "=========================================="
  run_n_times_and_average "$RESULTS_FILE" "$model" python script/train_task3.py \
    --model "$model" \
    --epochs "$EPOCHS" \
