#!/bin/bash
# Run the original Mamba model plus Mamba pooling ablations in parallel for Task 2.
...
if [ "${1:-}" = "-h" ] || [ "${1:-}" = "--help" ]; then
  echo "Usage: bash script/train_task2_mamba_ablation_parallel.sh"
  echo ""
  echo "Runs 4 jobs in parallel for Task 2:"
...
SELECTED_SCRIPT="script/train_task2_selected_models.sh"
SUMMARY_FILE="results/task2_mamba_ablation_parallel_${TIMESTAMP}.txt"
...
start_job \
  "mamba" \
  "results/log/mamba_task2_${TIMESTAMP}.log" \
  env \
...
start_ablation_job \
  ...
  "mamba_mean" \
  "${JOB_DEVICES[1]}" \
  "results/log/mamba_mean_task2_${TIMESTAMP}.log"

start_ablation_job \
  "mamba_max" \
  "mamba_max" \
  "${JOB_DEVICES[2]}" \
  "results/log/mamba_max_task2_${TIMESTAMP}.log"

start_ablation_job \
  "mamba_meanmax" \
  "mamba_meanmax" \
  "${JOB_DEVICES[3]}" \
  "results/log/mamba_meanmax_task2_${TIMESTAMP}.log"
...
{
  echo "Task 2 - Parallel Mamba Ablation Launcher"
  echo "Generated: $(date)"
  echo "Wrapper log: $WRAPPER_LOG"
  echo "Python: $PYTHON_BIN"
  echo "N_RUNS=$N_RUNS"
